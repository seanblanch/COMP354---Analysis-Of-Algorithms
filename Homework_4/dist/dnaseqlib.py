...
try:
    ...
    print("You don't have Pillow (the Python Imaging Library) installed.")
    print("Please check README.txt for instructions on how to install Pillow.")
    sys.exit(-1)
...
def compareSequences(getExactSubmatches, imgfile, imgsize, afile, bfile, k):
    a = kfasta.FastaSequence(afile)
    b = kfasta.FastaSequence(bfile)
    matches = getExactSubmatches(a, b, k)
